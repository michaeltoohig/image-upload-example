...
recreate-local-db:
	...
	$(DOCKER_COMPOSE) exec postgresql bash -c "echo 'drop database dbase' | psql $(PG_URI) && echo 'create database dbase' | psql $(PG_URI)"
	$(DOCKER_COMPOSE) start backend
	$(DOCKER_COMPOSE) exec backend bash -c "source venv/bin/activate && flask db upgrade"

psql:
	$(DOCKER_COMPOSE) exec postgresql psql $(PG_URI)/dbase
...
backend-bash:
	# bash shell that is configured to use 'dbase' database,
